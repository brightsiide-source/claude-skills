...
export default function AboutPage() {
  return (
    <>
      ...
      <section className="relative surface-paper pb-24">
        <div className="mx-auto max-w-4xl px-6 lg:px-10">
          <Reveal>
            <p className="text-ink-950 text-lg leading-relaxed text-pretty">
          ...
          <Reveal delay={0.1}>
            <p className="text-ink-950 text-lg leading-relaxed mt-8 text-pretty">
      ...
      <section className="relative surface-paper-warm pb-32 pt-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <Reveal>
            <h2 className="text-display-3 font-display text-ink-950 mb-12">The founders</h2>
          </Reveal>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {team.map((m, i) => (
              <Reveal key={m.name} delay={i * 0.08}>
                <div className="rounded-3xl card-paper p-8">
                  <div className="aspect-square rounded-2xl bg-gradient-to-br from-leaf-700 via-leaf-500 to-leaf-300 mb-6 grid place-items-center text-7xl text-ink-950 font-display font-black">
                    {m.name[0]}
                  </div>
                  <div className="text-leaf-700 text-xs uppercase tracking-widest mb-2 font-bold">
                    {m.role}
                  </div>
                  <h3 className="font-display text-ink-950 text-2xl font-bold mb-3">{m.name}</h3>
                  <p className="text-paper-muted text-sm leading-relaxed">{m.bio}</p>
                </div>
              </Reveal>
            ))}
          </div>
