    ...
    <>
            ...
            {tiers.map((t, i) => (
              <Reveal key={t.name} delay={i * 0.08}>
                ...
                >
                  ...
                  >
                    <div className="text-leaf-300 text-xs uppercase tracking-widest mb-2">
                      {t.pts}
                    </div>
                    ...
                    <ul className="space-y-3">
                      {t.perks.map((p) => (
                        <li
                          key={p}
                          className="flex items-start gap-3 text-ink-200 text-sm"
                        >
      ...
      <section className="relative py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <Reveal>
            <h2 className="text-display-3 font-display text-cream mb-4 text-balance">
              How Hash Pass <span className="gradient-text">works.</span>
            </h2>
            <p className="text-ink-200 max-w-2xl mb-12 text-pretty">
            ...
            {benefits.map((b, i) => (
              <Reveal key={b.title} delay={i * 0.06}>
                <div className="rounded-3xl glass p-8 h-full hover:border-leaf-500/20 transition-colors">
                  ...
                  <h3 className="font-display text-cream text-xl font-bold mb-3">{b.title}</h3>
                  <p className="text-ink-300 text-sm leading-relaxed text-pretty">{b.body}</p>
                </div>
              </Reveal>
            ))}
          </div>
