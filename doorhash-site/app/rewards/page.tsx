...
export default function RewardsPage() {
  return (
    <>
      ...
      <section className="relative surface-dark pb-12">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <Reveal>
            <h2 className="text-display-3 font-display text-white mb-4 text-balance">
              Choose your <span className="text-leaf-300">tier.</span>
            </h2>
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {tiers.map((t, i) => {
              ...
              const innerClass = t.featured
                ...
                : t.vip
                ? "rounded-[1.4rem] bg-black p-8 h-full"
                : "rounded-3xl card-dark p-8 h-full";
              const ptsClass = t.featured
                ...
                : "text-leaf-300 text-xs uppercase tracking-widest mb-2 font-bold";
              const titleClass = "font-display text-white text-3xl font-bold mb-6";
      ...
      <section className="relative surface-dark py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <Reveal>
            <h2 className="text-display-3 font-display text-white mb-4 text-balance">
              How Hash Pass <span className="text-leaf-300">works.</span>
            </h2>
            <p className="text-white/70 max-w-2xl mb-12 text-pretty">
              ...
              <Reveal key={b.title} delay={i * 0.06}>
                <div className="rounded-3xl card-dark p-8 h-full">
                  <div className="w-12 h-12 rounded-2xl bg-leaf-500/15 border border-leaf-500/30 grid place-items-center mb-6">
                    <b.icon className="w-5 h-5 text-leaf-300" />
                  </div>
                  <h3 className="font-display text-white text-xl font-bold mb-3">{b.title}</h3>
                  <p className="text-white/70 text-sm leading-relaxed text-pretty">{b.body}</p>
                </div>
              </Reveal>
            ))}
          </div>
