    ...
    <>
      ...
      <section className="relative surface-dark py-16 pb-32">
        <div className="mx-auto max-w-4xl px-6 lg:px-10">
          <Reveal>
            <h2 className="text-display-3 font-display text-white mb-12 text-center text-balance">
              More <span className="text-leaf-300">good questions.</span>
            </h2>
          </Reveal>
          <div className="space-y-4">
            {more.map((f, i) => (
              <Reveal key={f.q} delay={i * 0.04}>
                <details className="group rounded-2xl card-dark p-6">
                  <summary className="cursor-pointer flex items-center justify-between text-white font-display font-bold text-lg">
                    {f.q}
                    <span className="ml-4 text-leaf-300 group-open:rotate-45 transition-transform text-2xl leading-none">
                      +
                    </span>
                  </summary>
                  <p className="mt-4 text-white/70 leading-relaxed text-pretty">{f.a}</p>
                </details>
              </Reveal>
            ))}
          </div>
