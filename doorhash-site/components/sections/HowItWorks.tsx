      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <Reveal className="text-center max-w-2xl mx-auto mb-20">
          ...
          <h2 className="text-display-2 font-display text-cream text-balance">
            Order in <span className="gradient-text">under a minute.</span>
          </h2>
          <p className="text-lg text-ink-200 mt-6 text-pretty">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {steps.map((s, i) => (
            <Reveal key={s.title} delay={i * 0.08}>
              <div className="relative h-full rounded-3xl glass p-8 hover:border-leaf-500/20 transition-colors group">
                <div className="absolute top-6 right-6 text-7xl font-display font-black text-white/[0.04] leading-none">
                  0{i + 1}
                </div>
                <div className="relative">
                  <div className="w-12 h-12 rounded-2xl bg-leaf-500/10 border border-leaf-500/20 grid place-items-center mb-6 group-hover:bg-leaf-500/20 transition-colors">
                    <s.icon className="w-5 h-5 text-leaf-300" />
                  </div>
                  <h3 className="font-display text-cream text-xl font-bold mb-3">{s.title}</h3>
                  <p className="text-ink-300 text-sm text-pretty leading-relaxed">{s.body}</p>
                </div>
              </div>
            </Reveal>
          ))}
